...
regardless of naming; a class needs a `function` factory wrapper to be reachable at all. `process()`
takes plain config data (e.g. `GeminiConfig`) instead of a constructed `LLM` instance so provider
classes like `GeminiLLM` stay internal and never need to cross the library boundary.
